...
from config.BRADx_config import *
...
@router.get("/chiller-heater-shaker_ver/{id}")
...
@router.get("/heater_shaker_reset/{id}")
async def get_version(id: int):
    """Returns the version info of chiller / heater-shaker"""
    if id not in [      
        PREP_HEATER_SHAKER         
        ...
        raise HTTPException(
            status_code=500, detail=f"Heater-shaker ID {id} not available"
        )
    # Build the request message and packet
    message = BRADXRawRequest("reset")
    req = BRADxBusPacket(
        PREP_DECK_SUBSYSTEM_ID, id, message.raw, 30, BRADxBusPacketType.REQUEST
    )
    # Send the request and get the response
...
@router.get("/heater_shaker_shake_on/{id}")
async def get_shake_on(id: int):
    """Returns the version info of chiller / heater-shaker"""
    if id not in [      
        PREP_HEATER_SHAKER         
    ]:
...
@router.get("/heater_shaker_shake_off/{id}")
async def get_shake_off(id: int):
    """Returns the version info of chiller / heater-shaker"""
    if id not in [      
        PREP_HEATER_SHAKER         
    ]:
...
@router.get("/heater_shaker_shake_rpm/{id}")
async def get_shake_rpm(id: int):
    """Returns the version info of chiller / heater-shaker"""
    if id not in [      
        PREP_HEATER_SHAKER         
    ]:
...
@router.get("/heater_shaker_shake_max_rpm/{id}")
async def get_shake_max_rpm(id: int):
    """Returns the version info of chiller / heater-shaker"""
    if id not in [      
        PREP_HEATER_SHAKER         
    ]:
...
@router.post("/heater_shaker_set_rpm/{id}")
async def set_heater_rpm(
    id: int,
    set_rpm: int = Query(description="Set RPM"),
):
    ...
    # Build the request message and packet
    message = BRADXRequest(PREP_BUS_ADDR[id], rand_request_id(), "ssts1300", [])
    req = BRADxBusPacket(
        PREP_DECK_SUBSYSTEM_ID, id, message.raw, 30, BRADxBusPacketType.REQUEST
    )
    # Send the request and get the response
...
async def set_heater_time(
    ...
    # Build the request message and packet
    message = BRADXRequest(PREP_BUS_ADDR[id], rand_request_id(), "set", [str(set_time)])
    req = BRADxBusPacket(
        PREP_DECK_SUBSYSTEM_ID, id, message.raw, 30, BRADxBusPacketType.REQUEST
    )
    # Send the request and get the response
